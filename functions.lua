  ...
  for i,ds in ipairs(machine.datasets) do
    ...
    if ds.dom=="spec" then
      ...
      for k=1,n do
        ...
        if (ds.typ~='demod' and v > g_peak_threshold) or (ds.typ=='demod' and (v-mv)/mv>.5) then
        ...
        if typ=="normal" --[[or typ=="average"]] or typ=="demod" then -- Just some insurance we work with known tables.
          for ax, tdsi in pairs(axes) do
            ...
            elseif typ=='demod' then
              -- This code corrects AQ-204 demod frequency data. Remove when the data base is fixed.
              for _,dsi in ipairs(tdsi) do
                local data=machine.datasets[dsi]
                if #data.data==800 then
                  local fmax=40960/2.56/(round(40960/2.56/data.fmax))
                  data.fmax=fmax
                  data.bw=fmax/800
                end
              end
...
-- build a list of line frequency harmonics     
          ...
          for ax, pl in pairs(axes) do
            ...
            if pl.shaftspeed~=nil then
              ...
              for pi,pk in ipairs(pl.peaks) do
                ...
                if pi+1<#pl.peaks then
                  local nisharm,nm,nr=is_a_multiple(machine.linef*2,pl.peaks[pi+1].sfreq,close)
                  nextiscloser=nisharm and nm==m and nr<r
                end
                if isharm and lford/bw>3 and m<=15 and not (nextiscloser) and not(speedissync) and (rr<.005 or 1-r<.001) and m~=lastlf then
        ...
        for typ,axes in pairs(e.data.spec.cpl) do
          for ax, pl in pairs(axes) do
            ...
            --for pi,sp in ipairs(speeds) do -- removed apparent duplicate 4/11/19
              local subs
              for _,sp in ipairs (speeds) do
                if #speeds==1 then
        ...
        local close=.008
        local dmaxes={}   -- add binsearchpl(pl,target,delta,key,speedratio) to speed it search of normal PL for matches to the demaod peaks.
        local dmaxis=''
        for typ,axes in pairs(e.data.spec.cpl) do
          if typ=='demod' then
            ...
            for pi,pk in ipairs(pl.peaks) do
              ...
              if typ=="normal" and 20*math.log(pk.mpct,10)>3 then
                ...
                local aq204=false
                  ...
                  --if daxes~=nil then
                  --if daxes==ax then 
                  local startindex=binsearchpl(dmpl,pk.sfreq,dmbw,'sfreq')
                  for indx=startindex,#dmpl.peaks,1 do
                    --for _,dmpk in ipairs(dmpl.peaks) do
                    local dmpk=dmpl.peaks[indx]
                    if dmpk~=nil then 
                      local matched=string.match(dmpk.flags,'a'..ax)=='a'..ax
                      if not (matched) then

                        local dmfreq=dmpk.sfreq
                        --if aq204 then close=close*1.5 end
                        local isclose, m1, d1 =is_a_multiple(dmfreq,pk.sfreq,close)
                      if isclose and m1==1 and dmpk.dif~=nil and dmpk.dif>1 then -- is this demod peak freq nearly equal to current peak freq
                          if pi+1<=#pl.peaks then 
                            local alsoclose, m2, d2 =is_a_multiple(dmfreq,pl.peaks[pi+1].sfreq,close)
                            if alsoclose and m2==1 and 20*math.log(pl.peaks[pi+1].mpct,10)>3 then  -- is the next peak closer to this demod peak freq
                          ...
                          if dmpk.sfreq > pk.sfreq*1.08 then goto nextpk end
                        end 
                      end
                    end  
                    --end 
              ...
              for s,spd in ipairs(speeds) do
                ...
                if ( pk.sord>sb*spd*2 and not (lf)) or (pk.sfreq>10*machine.linef and lf) or typ=='demod' or not (isshaftmult) then
                  local start=binsearchpl(pl,(pk.sord-(sb*spd))*1.11,0,'sord',1)
                  for spi=start,#pl.peaks,1 do
                    local spk=pl.peaks[spi]
                    --for spi,spk in ipairs(pl.peaks) do
                    if spk.sord>(pk.sord+(sb*spd))*1.11 then break end
                    if spk.sord>(pk.sord-(sb*spd))*1.10 and ((lf and spk.sfreq>10*machine.linef) or not (lf)) then
                      local bw
                      if typ=='average' then
                      ...
                      for sbd=-sb,sb,1 do
                        ...
                        if issame and spd/bw>3 and m==1 and not (nextiscloser)  then
                          if spk.sev~=nil and spk.sev>0 then ssev=ssev+spk.sev end
                ...
                if #sides>1  then
                  ...
                  if ssev~=nil and ssev>0 then pk.sb_severity[sbindex]=ssev end
                end
              end
            end -- peak
            for pi,pk in ipairs(pl.peaks) do
...
--
    ...
    local function asn_machine( w )
      local class = machine.class
      local str = '<machine '
      str = str .. "name" .. '=\"' .. machine.name .. '\" ' 
      if class~=nil and string.len(class)>0 then
        str = str .. "class" .. '=\"' .. class .. '\" ' 
      end
      str = str .. "ispeed" .. '=\"' .. machine.ispeed .. '\" '   
      str = str .. "speed" .. '=\"' .. machine.speed .. '\" ' 
      str = str .. "vspeed" .. '=\"' .. machine.vspeed .. '\" ' 
      str = str .. "scale" .. '=\"' .. machine.scale .. '\" ' 
      str = str .. "linef" .. '=\"' .. machine.linef .. '\" ' 
      str = str .. '/>'
    ...
    local function asn_data( k, t, w )
      local ls
      local name = machine.elements[k].name
      if name~=nil and string.len(name) > 0 then
        ls = {xml='<mel val=\"'..k..'\" guid=\"'.. machine.elements[k].guid ..'\" name=\"'.. name ..'\" >' }
      else
        ls = {xml='<mel val=\"'..k..'\" guid=\"'.. machine.elements[k].guid ..'\" >' }
      end
...
local function asn_comp_rule_map( k, w )
  ...
  w.xml = w.xml .. ls.xml
    ...
    local function asn_comp( k, t, w )
      local ls
      local name = machine.shafts[ machine.components[k].shaft ].name
      if( string.len( name ) > 0 ) then
        ls = {xml = '<comp val=\"'..k..'\" name=\"'..name..'\">' }
      else
        ls = {xml = '<comp val=\"'..k..'\">' }
      end
...
local function asn_element( k, t, w )
  ...
  local name = machine.elements[k].name
  if name~=nil and string.len(name) > 0 then
    ls = {xml = '<mel val=\"'..k..'\" guid=\"'..t.guid..'\" ratio=\"'..t.speedratio..'\" name=\"'..name..'\" >' }  
  else
    ls = {xml = '<mel val=\"'..k..'\" guid=\"'..t.guid..'\" ratio=\"'..t.speedratio..'\" >' }  
  end
...
--
    ...
    function find_tag_info_cpl( tag, shaft_tag_index, shaft_data_index, axis, composite_shaft_index )
      ...
      --REVIEW: copy p.pos to ret.  This is the peak list index.  (check to make sure it is pos).
      --        This will be useful to get back to the peak in the peak list.
      if ret.aval then
